...
class OptimizationResult:
    def __init__(self, params: EnergyMixParams, cost: float, p95_hours_empty: float, total_hours: int, energy_surplus: float):
        self.params = params
        self.cost = cost
    ...
    def p95_empty_percent(self) -> float:
        return (self.p95_hours_empty / self.total_hours) * 100.0 if self.total_hours > 0 else 0.0
    ...
    def __repr__(self):
        ...
        return (f"[{valid_str}] Cost: {self.cost:.1f}, "
...
def mutate_params(result: OptimizationResult, bounds: Dict[str, Tuple[int, int]]) -> EnergyMixParams:
    """
    ...
    if result.energy_surplus < 0:
        ...
        bias = 1 # Increase
        prob_bias = 0.8
        fields = ["power_wheels", "water_wheels", "large_windmills", "windmills", "batteries", "battery_height"]
    else:
...
def optimize(
    ...
    current_result = evaluate_config(base_params, current_params, simulations_per_config, total_hours)
    ...
    for i in range(iterations):
        ...
        candidate_result = evaluate_config(base_params, candidate_params, simulations_per_config, total_hours)
        ...
        if candidate_score < current_score:
            ...
            if current_result.is_valid:
                if best_valid_result is None or current_result.cost < best_valid_result.cost:
        ...
        if (i + 1) % 10 == 0:
            print(f"Iteration {i + 1}/{iterations}. Current Score: {current_score:.0f}. Best Valid Cost: {best_valid_result.cost if best_valid_result else 'None'}")

    return history

def find_optimal_solutions(results: List[OptimizationResult], max_empty_percent: float = 5.0) -> List[OptimizationResult]:
    """
    ...
    for sol in valid_solutions:
        ...
        p = sol.params
        key = (p.power_wheels, p.water_wheels, p.large_windmills, p.windmills, p.batteries, p.battery_height)
