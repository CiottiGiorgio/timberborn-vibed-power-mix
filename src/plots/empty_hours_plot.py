...
def plot_empty_hours(ax, run_empty_hours, total_runs):
    ...
    if run_empty_hours:
        ...
        # Plot the histogram
        n, bins, patches = ax.hist(run_empty_hours, bins=bins, color='red', alpha=0.7, edgecolor='black')
        ...
        if run_empty_hours:
            ...
            ax.axvline(p5, color='blue', linestyle='--', linewidth=1.5, label=f'5th % ({p5:.1f}h)')
            ax.axvline(p50, color='green', linestyle='--', linewidth=1.5, label=f'Median ({p50:.1f}h)')
            ax.axvline(mean_val, color='orange', linestyle='-', linewidth=1.5, label=f'Mean ({mean_val:.1f}h)')
            ax.axvline(p95, color='purple', linestyle='--', linewidth=1.5, label=f'95th % ({p95:.1f}h)')
